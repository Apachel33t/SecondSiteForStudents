...
class Contact extends ConnectDB
{
    ...
    public function createContact($customer_id, $type_id, $contact)
    {
        ...
        try {
            if (!parent::$sql->real_query("INSERT INTO study_site.contacts (customer_id, contacts_type_id, contact) values ('$customer_id', '$type_id', '$contact');"))
            {
    ...
    {
        parent::$sql->real_query("SELECT * FROM study_site.contacts where customer_id = '$customer_id'");
    }

    public function returnContacts()
    {
        parent::$sql->real_query("SELECT * FROM study_site.contacts");
    }

    public function deleteContactByCustomerId($customer_id)
    {
        parent::$sql->real_query("DELETE FROM study_site.contacts WHERE customer_id = '$customer_id'");
    }
}